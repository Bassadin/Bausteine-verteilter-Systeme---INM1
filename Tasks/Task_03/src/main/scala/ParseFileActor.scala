import ConvertDataActor.Terminate
...
object ParseFileActor {
    ...
    case class ListingResponse(listing: Receptionist.Listing) extends ParseFileActorProtocol
    case class AskForWork(converterRef: ActorRef[ConvertDataActor.ConvertDataActorProtocol])
        extends ParseFileActorProtocol

    val serviceKey: ServiceKey[ParseFileActorProtocol] = ServiceKey[ParseFileActorProtocol]("fileParser")
    ...
    def apply(csvPath: String): Behavior[ParseFileActorProtocol] =
        Behaviors.setup { context =>
            ...
            context.system.receptionist ! Receptionist.Subscribe(ConvertDataActor.serviceKey, subscriptionAdapter)
